...
namespace molar {
    constexpr auto resource_tokens =
        std::array{TokenType::Geometry, TokenType::Material, TokenType::Texture};

    constexpr auto call_ops = std::array{CallType::Math, CallType::Query};
    ...
    constexpr auto tokens_which_could_be_names_constructed = std::array{
        Token{TokenType::Math},      Token{TokenType::Query},    Token{TokenType::Variable},
        Token{TokenType::Geometry},  Token{TokenType::Material}, Token{TokenType::Texture},
        Token{TokenType::Array},     Token{TokenType::True},     Token{TokenType::False},
        Token{TokenType::This},      Token{TokenType::Break},    Token{TokenType::Continue},
        Token{TokenType::ForEach},   Token{TokenType::Loop},     Token{TokenType::Return},
        Token{TokenType::Temporary}, Token{TokenType::Context},  Token{TokenType::Identifier},
    };
    ...
    ast::ExpressionPtr MolangAstGenerator::build_expression_recursive(uint8_t min_binding_power
        ...
        while (true) {
            if (ast::ExpressionPtr ptr; this->try_parse_arrow(std::move(lhs_expression), ptr)) {
                ...
                lhs_expression = std::move(ptr
                ); // ptr always gets set to a valid value, in this cause our expression
            }

            uint8_t right_bind_power = 0xFF;
            try {
                if (const auto bind_power =
                        token_bind_power(Token(this->tokens.peek_value()).type);
                    bind_power) {
                    if (const auto [left_bp, right_bp] = bind_power.value();
                        left_bp < min_binding_power) {
                        break;
                    } else {
                ...
                break;
                // Swallow the error since it means we have finished the buffer (the last
                // expression was the last one)
            }

            auto [next_expression, matched] =
                this->next_expression(std::move(lhs_expression), right_bind_power);
    ...
    bool MolangAstGenerator::try_var(ast::ExpressionPtr& returns) {
        if (auto value = this->build_variable()) {
            if (this->tokens.next(Token{TokenType::Assign})) {
                ...
                if (!expression) {
                    throw MolangSyntaxError(
                        "Expected expression", value.value().get_position()
                    );
                }

                returns = std::make_unique<ast::VariableAssign>(
                    std::move(value.value()), std::move(expression)
                );
            } else {
    ...
    bool MolangAstGenerator::try_bool(ast::ExpressionPtr& returns) {
        if (const auto token = this->tokens.next_value(
                std::array{Token{TokenType::True}, Token{TokenType::False}}
            );
            token.has_value()) {
    ...
    bool MolangAstGenerator::try_number(ast::ExpressionPtr& returns) {
        if (const auto token = this->tokens.next_value(Token{TokenType::Number});
            token.has_value()) {
            returns =
                std::move(std::make_unique<ast::NumericLiteral>(token.value(), this->source));
            return true;
        }
    ...
    bool MolangAstGenerator::try_string(ast::ExpressionPtr& returns) {
        if (const auto token = this->tokens.next_value(Token{TokenType::String});
            token.has_value()) {
            returns =
                std::move(std::make_unique<ast::StringLiteral>(token.value(), this->source));
            return true;
        }
        return false;
    }
    ...
    ast::ExpressionPtr MolangAstGenerator::next_expression() {
        ...
        // TODO: Add a function which just checks if any invalid tokens are next. Basically
        // meaning that if there is something like an = next we throw. Since there is no valid
        // state where this could happen

        return nullptr;
    }

    // NOLINTNEXTLINE
    bool MolangAstGenerator::try_parse_binary(
        ast::ExpressionPtr&& lhs, std::pair<ast::ExpressionPtr, bool>& value,
        ...
        if (const Token next_token = next_tk; token_is_bin_op(next_token)) {
            ...
            next_tk.apply(
            ); // We don't need to bind this peek since the expression building consumes it

            auto rhs = this->build_expression_recursive(min_binding_power);
            value    = {
                std::make_unique<ast::BinaryExpression>(
                    next_token.position, next_token.size, op, std::move(lhs), std::move(rhs)
                ),
                true
            };
            return true;
    ...
    bool
    MolangAstGenerator::try_parse_arrow(ast::ExpressionPtr&& lhs, ast::ExpressionPtr& value) {
        if (const auto token_temp = this->tokens.next_value(Token{TokenType::Arrow});
            token_temp.has_value()) {
            ...
            value = std::move(std::make_unique<ast::ArrowAccess>(
                token.position, token.size, std::move(lhs), std::move(expr)
            ));
    ...
    bool MolangAstGenerator::try_parse_branch(
        ast::ExpressionPtr&& condition, ast::ExpressionPtr& value
    ) {
        if (const auto token_temp = this->tokens.next_value(Token{TokenType::Conditional});
            token_temp) {
            const auto token = token_temp.value();
            ...
            if (!if_expression) {
                throw MolangSyntaxError(
                    "Expected expression after conditional", token.position
                );
            }

            if (const auto temp_colon = this->tokens.next_value(Token{TokenType::Colon});
                temp_colon.has_value()) {
    ...
    bool MolangAstGenerator::try_parse_paren(ast::ExpressionPtr& returns) {
        size_t position{};
        if (const auto p = this->tokens.next(Token{TokenType::LeftParen}); !p.has_value())
            return false;
        ...
        returns = std::move(std::make_unique<ast::ParenthesizedExpression>(
            position, exprs.size(), std::move(exprs)
        ));
    ...
    std::optional<ast::BlockExpression> MolangAstGenerator::build_block() {
        ...
        if (const auto position = this->tokens.next(Token{TokenType::LeftBrace});
            !position.has_value())
            return std::nullopt;
        ...
        while (!this->tokens.next(Token{TokenType::RightBrace})) {
            ...
            if (!this->tokens.next({Token{TokenType::Semi}})) {
                throw MolangSyntaxError(
                    "Expected semi colon after expression in block", our_position
                );
            }
        }
    ...
    std::optional<ast::VariableReference> MolangAstGenerator::build_variable() {
        if (const auto token = this->tokens.next_value(std::array{
                Token{TokenType::Variable}, Token{TokenType::Context},
    ...
    bool MolangAstGenerator::try_parse_unary(ast::ExpressionPtr& returns) {
        if (const auto pos = this->tokens.next_value(
                std::array{Token{TokenType::Minus}, Token{TokenType::Not}}
            );
            pos.has_value()) {
            ...
            if (!expr) {
                throw MolangSyntaxError(
                    "Expected expression after a Unary Operator", pos.value().position
                );
            }

            returns = std::make_unique<ast::UnaryExpression>(
                pos.value().position, pos.value().size, static_cast<UnaryOp>(pos.value().type),
                std::move(expr)
            );
            return true;
        }
    ...
    bool MolangAstGenerator::try_parse_call(ast::ExpressionPtr& returns) {
        if (const auto current_token = this->tokens.next_value(
                std::array{Token{TokenType::Math}, Token{TokenType::Query}}
            );
            ...
            if (!this->tokens.next(Token{TokenType::Dot}))
                throw MolangSyntaxError(
                    "Expected Dot after Call Type (math, or query)",
            ...
            if (!ident_token.check_types(tokens_which_could_be_names)) {
                throw MolangSyntaxError(
                    "Expected identifier after Call Type", ident_token.position
                );
            }
            ...
            if (this->tokens.next(Token{TokenType::LeftParen})) {
                try {
                    while (true) {
                        ...
                        if (!expr) {
                            throw MolangSyntaxError(
                                "Invalid expression in call list", ident_token.position
                            );
                        }
                    ...
                    // This means we EOF
                    throw MolangSyntaxError(
                        "Unexpected End Of File at call expression", ident_token.position
                    );
                }
            }

            returns = std::move(std::make_unique<ast::CallExpression>(
                current_token.value().position, current_token.value().size, kind,
    ...
    bool MolangAstGenerator::try_parse_resource(ast::ExpressionPtr& returns) {
        ...
        if (!ident_token.check_types(tokens_which_could_be_names)) {
            throw MolangSyntaxError(
                "Expected identifier after resource Type", ident_token.position
            );
        }

        auto identifier = ast::IdentifierLiteral(ident_token, this->source);

        returns = std::move(std::make_unique<ast::ResourceExpression>(
            token.position, token.size, static_cast<ResourceKind>(token.type),
            std::move(identifier)
        ));
    ...
    bool MolangAstGenerator::try_parse_array(ast::ExpressionPtr& returns) {
        if (const auto token_temp = this->tokens.next_value(Token{TokenType::Array});
            token_temp.has_value()) {
            const auto token = token_temp.value();
            if (!this->tokens.next(Token{TokenType::Dot})) {
            ...
            if (const auto identifier_temp =
                    this->tokens.next_value(tokens_which_could_be_names_constructed);
                identifier_temp.has_value()) {
                ident =
                    std::move(ast::IdentifierLiteral(identifier_temp.value(), this->source));
            } else {
            ...
            returns = std::move(std::make_unique<ast::ArrayAccess>(
                token.position, token.size, std::move(ident), std::move(index_expression)
            ));
            return true;
        }
        return false;
    ...
    bool MolangAstGenerator::try_parse_loop(ast::ExpressionPtr& returns) {
        if (const auto temp_token = this->tokens.next_value(Token{TokenType::Loop});
            temp_token.has_value()) {
            ...
            if (!this->tokens.next(Token{TokenType::RightParen})) {
                throw MolangSyntaxError(
                    "Expected right paren after expression", token.position
                );
            }

            returns = std::make_unique<ast::LoopExpression>(
                token.position, token.size, std::move(count_expression),
    ...
    bool MolangAstGenerator::try_parse_for_each(ast::ExpressionPtr& returns) {
        if (const auto temp_token = this->tokens.next_value(Token{TokenType::ForEach});
            temp_token.has_value()) {
            const auto token = temp_token.value();
            ...
            if (!this->tokens.next(Token{TokenType::Comma})) {
                throw MolangSyntaxError(
                    "Expected Comma after variable declaration", token.position
                );
            }
            ...
            returns = std::move(std::make_unique<ast::ForEachExpression>(
                token.position, token.size, std::move(variable.value()),
                std::move(array_fetch_expression), std::move(for_each_code.value())
            ));
    ...
    bool MolangAstGenerator::try_parse_keyword(ast::ExpressionPtr& returns) {
        if (const auto temp_token = this->tokens.next_value(Token{TokenType::Break});
            temp_token.has_value()) {
            const auto token = temp_token.value();
        ...
        if (const auto temp_token = this->tokens.next_value(Token{TokenType::Continue});
            temp_token.has_value()) {
            const auto token = temp_token.value();
            returns =
                std::move(std::make_unique<ast::ContinueNode>(token.position, token.size));
            return true;
        }

        if (const auto temp_token = this->tokens.next_value(Token{TokenType::This});
            temp_token.has_value()) {
            const auto token = temp_token.value();
    ...
    bool MolangAstGenerator::try_parse_return(ast::ExpressionPtr& returns) {
        if (const auto temp_token = this->tokens.next_value(Token{TokenType::Return});
            temp_token.has_value()) {
            const auto token = temp_token.value();
            auto       expr =
                this->build_expression_recursive(0); // We don't care if this is a nullptr
            returns = std::move(
                std::make_unique<ast::ReturnNode>(token.position, token.size, std::move(expr))
            );
            return true;
        }
    ...
    // This function builds any "complex" expressions, such as a binary operation since that
    // needs the last operation NOLINTNEXTLINE
    std::pair<ast::ExpressionPtr, bool> MolangAstGenerator::next_expression(
        ...
        try {
            ...
            if (std::pair<ast::ExpressionPtr, bool> value;
                try_parse_binary(std::move(lhs), value, min_binding_power))
                return value;
        } catch (std::out_of_range&) {
    ...
    bool MolangAstGenerator::token_is_bin_op(const Token& token) {
        constexpr auto bin_ops = std::array{
            BinaryOp::Equality,      BinaryOp::Inequality,  BinaryOp::LessThan,
            BinaryOp::LessEqualThan, BinaryOp::GreaterThan, BinaryOp::GreaterEqualThan,
            BinaryOp::Addition,      BinaryOp::Subtraction, BinaryOp::Multiplication,
            BinaryOp::Division,      BinaryOp::Or,          BinaryOp::And,
            BinaryOp::Coalesce,
        };
