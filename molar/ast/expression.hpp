...
namespace molar::ast {
    enum class AstKind {
        ...
        // Only ever show up after processing, meaning a visitor shouldn't handle then ever?

        PreAllocatedVariableReference,
        PreAllocatedAssignment,
        PreAllocatedCall,
        PreAllocatedForLoop
    };
    ...
    public:
        ...
        RawExpression(AstKind type) : type(type) {}
    ...
    public:
        ...
        Expression(const size_t position, const size_t size, const AstKind type)
            : RawExpression(type), position(position), size(size) {}

        explicit Expression(const size_t position, const AstKind type)
            : RawExpression(type), position(position), size(1) {}

        explicit Expression(const Token& token, const AstKind type)
            : RawExpression(type), position(token.position), size(token.size) {}
    ...
    public:
        ParenthesizedExpression(
            const size_t position, const size_t size, RawExpressionList&& expressions
        )
            : Expression(position, size, AstKind::ParenthesizedExpression),
              expressions(std::move(expressions)) {}

        [[nodiscard]] RawExpressionList&       get_expressions() { return this->expressions; }
        [[nodiscard]] const RawExpressionList& get_expressions() const {
    ...
    public:
        BlockExpression(
            ...
            : ParenthesizedExpression(position, size, std::move(expressions)) {
    ...
    public:
        BinaryExpression(
            const size_t position, const size_t size, const BinaryOp operation,
            RawExpressionPtr&& left, RawExpressionPtr&& right
        )
            : Expression(position, size, AstKind::BinaryExpression), left(std::move(left)),
              right(std::move(right)), operation(operation) {}
    ...
    public:
        UnaryExpression(
            const size_t position, const size_t size, const UnaryOp operation,
            RawExpressionPtr&& expression
        )
            : Expression(position, size, AstKind::UnaryExpression), operation(operation),
